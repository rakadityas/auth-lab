...
// notify sends a security notification to a user. Users in this lab are bare
// ids ("alice"); map them to a deliverable address so Mailpit catches the mail.
func notify(user, subject, body string) {
	to := user
	for _, c := range user {
		if c == '@' {
	...
	sendMail(to+"@auth-lab.test", subject, body)
}
